...
final class SheetScanFixtureTests: XCTestCase {
            ...
            for (key, expected) in sheet.columns.sorted(by: { $0.key < $1.key }) {
                ...
                for index in expected.indices where result.entries[index] != expected[index] {
                    ...
                    let raw = (reading.readings[.entry(index)] ?? []).map { "\($0.mark)@\(String(format: "%.2f", $0.confidence))" }
                        ...
                        + (reading.guessed.contains(.entry(index)) ? ["digit net"] : [])
                ...
                for line in reading.guessed {
                    guard case .entry(let index) = line else { continue }
                    tally.netGuesses += 1
                    if result.entries[index] == expected[index] { tally.netRight += 1 }
                }
    ...
    private struct Tally {
        ...
        /// Boxes only the digit net could read, and how many came out right.
        var netGuesses = 0, netRight = 0
        ...
        var summary: String {
            ...
            return "\(correct)/\(boxes) boxes (\(String(format: "%.1f", accuracy))%), \(perfect)/\(columns) columns perfect, "
                + "\(silent) wrong without a flag, \(flagged) flagged, digit net \(netRight)/\(netGuesses) right"
        }
    }
}
